...
public class WordLadder {
  ...
  public static void main(String[] args) {
    ...
    String[] words =
        new String[] {
          "ricky", "grind", "cubic", "panic", "lover", "farce", "gofer", "sales", "flint", "omens",
    ...
    int min = wordLadder.ladderLength("nanny", "aloud", wordList);
    System.out.println("Min distance required :" + min);
  }

  /**
   * Algo : process words(hamming distance =1) and which are present in wordList and keep putting in
   * queue break when endWord encountered.
   */
      ...
      for (int s = 0; s < size; s++) {
        ...
        for (int i = 0; i < word.length(); i++) {
          ...
          for (char ch = 'a'; ch <= 'z'; ch++) {
            ...
            if (wordList.contains(converted)) {
              wordList.remove(converted);
              queue.add(converted);
              if (converted.equals(endWord)) return level + 1;
  ...
  /**
   * Algo : Build a adj list using the current word and words having hamming distance=1 Keep
   * processed nodes in a list to avoid duplication/cycle.
   */
  public int ladderLength_inefficient(String beginWord, String endWord, List<String> wordList) {
    ...
    for (int i = 0; i < wordList.size(); i++) {
      ...
      if (isHammingWord(beginWord, compare)) list.add(compare);
    }
    hamming.put(beginWord, list);
    int min = bfs(hamming, beginWord, endWord, new ArrayList<>());
    return min;
  }

  // O(h)*O(v) : where h is the max height of tree and v is the max number of neighbour nodes
  private int bfs(Map<String, List<String>> hamming, String start, String end, List<String> path) {
    ...
    while (!queue.isEmpty()) {
      ...
      for (int i = 0; i < size; i++) {
        List<String> neighbour = hamming.get(queue.poll());
        for (int j = 0; j < neighbour.size(); j++) {
          String next = neighbour.get(j);
